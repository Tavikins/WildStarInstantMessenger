...
function WildStarInstantMessenger:OnDocLoaded()
	...
	if self.xmlDoc ~= nil and self.xmlDoc:IsLoaded() then
		...
		self.eType = {
			Whisper = ChatSystemLib.ChatChannel_Whisper,
			Account = ChatSystemLib.ChatChannel_AccountWhisper,
		}
...
function WildStarInstantMessenger:FindAccountName(strCharName)
	local strSender = strCharName
	local tAccountFriends = FriendshipLib.GetAccountList()
	for idx, tAccountFriend in pairs(tAccountFriends) do
		if tAccountFriend.arCharacters ~= nil and tAccountFriend.arCharacters[1] ~= nil then
			for idx, tCharacter in pairs(tAccountFriend.arCharacters) do
				if tCharacter.strCharacterName == strCharName then 
					strSender = tAccountFriend.strCharacterName
				return strSender
				end
			end
		end
...
function WildStarInstantMessenger:OnChatMessage(channelCurrent, tMessage)
	...
	local strRealm = tMessage.strRealmName
	local eType = channelCurrent:GetType()
	if eType == self.eType.Whisper or eType == self.eType.Account then
		if eType == self.eType.Account then
			strName = self:FindAccountName(strName)
		elseif strRealm:len() > 0 and eType ~= self.eType.Account then
			strName = tMessage.strSender.."@"..tMessage.strRealmName
		else
			strName = tMessage.strSender
		end
	
		if self.wndWhispers[strName] == nil then
			self.tNotifications[strName] = {
				["strName"] = strName,
				["bNewMsg"] = false
			}
			self:PopWhisper(channelCurrent, tMessage)
			self:AddToChatWindow(channelCurrent, tMessage, true)
		elseif self.wndWhispers[strSender]:IsVisible() then
			self:AddToChatWindow(channelCurrent, tMessage)
		elseif self.db.profile.general.bShowEvery then
			if not self.bInCombat and self.db.profile.general.bHideCombat then
				self.wndWhispers[strName]:Show(true)
			end
			self:AddToChatWindow(channelCurrent, tMessage)
		else
			self.tNotifications[strName]["bNewMsg"] = true
			self:AddToChatWindow(channelCurrent, tMessage)
			self:UpdateNotificationAlert(self)
			--Show Prev
			self:ShowMsgPreview(tMessage)
		end
	else
		...
		elseif eType == ChatSystemLib.ChatChannel_Trade and self.db.profile.chatChannels.bShowTradeChat then
		elseif eType == ChatSystemLib.ChatChannel_GuildOfficer and self.db.profile.chatChannels.bShowGuildOfficerChat then
		elseif eType == ChatSystemLib.ChatChannel_ZonePvP and self.db.profile.chatChannels.bShowZonePvPChat then
		else
			return
		end
	
		if self.wndWhispers[channelCurrent:GetName()] == nil then
			self.tNotifications[channelCurrent:GetName()] = {
				["strName"] = channelCurrent:GetName(),
				["bNewMsg"] = false
			}
			self:PopOther(channelCurrent)
			self:AddToChatWindow(channelCurrent, tMessage, true)
		elseif self.wndWhispers[channelCurrent:GetName()]:IsVisible() then
			self:AddToChatWindow(channelCurrent, tMessage)
		elseif self.db.profile.general.bShowEvery and not self.db.profile.general.bShowEveryWhisper then
			if not self.bInCombat and self.db.profile.general.bHideCombat then
				self.wndWhispers[channelCurrent:GetName()]:Show(true)
			end
			self:AddToChatWindow(channelCurrent, tMessage)
		else
...
function WildStarInstantMessenger:AddToChatWindow(channelCurrent, tMessage, bIsFirst)
	...
	if eType == self.eType.Whisper or eType == self.eType.Account then
		if eType == self.eType.Account then
			strSender = self:FindAccountName(strName)
			strName = strSender
		elseif strRealm:len() > 0 and eType ~= self.eType.Account then
			strSender = tMessage.strSender.."@"..tMessage.strRealmName
			strName = strSender
		else
	...
	if channelCurrent:GetType() == ChatSystemLib.ChatChannel_AccountWhisper then
		strSender = self:FindAccountName(tMessage.strSender)
	elseif tMessage.strRealmName:len() > 0 and not channelCurrent:GetType() == ChatSystemLib.ChatChannel_AccountWhisper then
		strSender = tMessage.strSender .. "@" .. tMessage.strRealmName
	end
...
function WildStarInstantMessenger:PopWhisper(channelCurrent, tMessage)
	...
	if self.db.profile.windowAppearance.bUseMinimalStyle then
		self.wndWhispers[strName] = Apollo.LoadForm(self.xmlDoc, "ChatDialogueMinimal", nil, self)
	else
		self.wndWhispers[strName] = Apollo.LoadForm(self.xmlDoc, "ChatDialogueTabbed", nil, self)
	end
	
	self.wndWhispers[strName]:SetStyle("Escapable", self.db.profile.general.bCloseOnEscape)
	
	self.wndWhispers[strName]:SetData(tData)
	
	self.wndWhispers[strName]:Show(false)
	
	self.wndWhispers[strName]:SetText(strSender)
	
	self.wndWhispers[strName]:SetOpacity(self.db.profile.windowAppearance.nOpacity)
	
	local l, t, r, b = self.wndWhispers[strName]:GetAnchorOffsets()
	self.wndWhispers[strName]:SetAnchorOffsets(l, t, l+self.db.profile.windowAppearance.nWidth, t+self.db.profile.windowAppearance.nHeight)
	
	if not self.db.profile.windowAppearance.bUseMinimalStyle then
		self.wndWhispers[strName]:FindChild("Title"):SetText(strName)
	end
	
	for name, wndChat in pairs(self.wndWhispers) do
		wndChat:FindChild("EditBox1"):SetData(false)
	end
	self.wndWhispers[strName]:FindChild("EditBox1"):SetData(true)
	--Level: 88 - Medic
	...
	WildStarInstantMessenger:AddLogToWindow(self, strName)
...
function WildStarInstantMessenger:OnInputReturn( wndHandler, wndControl, strText )
		...
		if strText ~= "" then
			local channelCurrent = tChatData.channelCurrent
			if channelCurrent:GetType() == ChatSystemLib.ChatChannel_Whisper then
				tInput.strMessage = strName.." "..tInput.strMessage
			elseif channelCurrent:GetType() == ChatSystemLib.ChatChannel_AccountWhisper then
				tInput.strMessage = strName.." "..tInput.strMessage
			end
